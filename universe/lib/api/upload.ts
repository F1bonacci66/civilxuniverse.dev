...
export async function uploadFile(
  ...
  const uploadUrl = `${API_BASE_URL}/upload/`  // Добавляем trailing slash, чтобы избежать 308 редиректа
  console.log('uploadFile вызван:', {
    fileName: request.file.name,
    fileSize: request.file.size,
    projectId: request.projectId,
    convertToIFC: request.convertToIFC,
    convertToIFCType: typeof request.convertToIFC,
    versionId: request.versionId,
    autoConvert: request.autoConvert,
    convertToIFC: request.convertToIFC, // Добавляем convertToIFC в логи
    uploadUrl,
    API_BASE_URL,
  })
  ...
  console.log('🔍 [uploadFile] Проверка convertToIFC:', {
    convertToIFC: request.convertToIFC,
    type: typeof request.convertToIFC,
    isUndefined: request.convertToIFC === undefined,
    isTrue: request.convertToIFC === true,
    isFalse: request.convertToIFC === false,
  })
  
  if (request.convertToIFC !== undefined) {
    const valueToSend = String(request.convertToIFC)
    formData.append('convertToIFC', valueToSend)
    console.log('✅ convertToIFC добавлен в FormData:', {
      original: request.convertToIFC,
      sent: valueToSend,
      type: typeof request.convertToIFC,
    })
  } else {
    console.log('⚠️ convertToIFC не определен в request:', {
      request: request,
      hasConvertToIFC: 'convertToIFC' in request,
    })
  }
